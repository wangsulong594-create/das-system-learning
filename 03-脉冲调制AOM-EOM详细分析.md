...
##### AOM 给它做了频移以后：
$$\[
E_{\text{1级光}}(t) = \cos\bigl(2\pi (f_0 + f_{\text{AOM}})\, t\bigr)
\]$$
...
2. **瑞利散射回波**：
   去过光纤，**被 AOM 移过频**
$[
   \cos\bigl(2\pi (f_0 + f_{\text{AOM}})\, t + \phi(t)\bigr)
   \]$$
   这里 $\(\phi(t)\)$ 就是振动带来的**相位变化**。


### 两束光一干涉，会发生什么？
$$\[
\cos(2\pi f_0 t) \times \cos\bigl(2\pi (f_0 + f_{\text{AOM}}) t + \phi(t)\bigr)
\]$$
...
$$\[
\cos\bigl(2\pi f_{\text{AOM}} \,t + \phi(t)\bigr)
\]$$
...
- 没有频移：
  两束光频率一样，干涉出来是**直流/低频**，被 1/f 噪声淹死。

- 有频移：
  信号被**扛到高频 f_AOM**（比如 80MHz、100MHz、200MHz）
  远离低频噪声。
